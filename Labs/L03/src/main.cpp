...
static void init()
{
	...
	// Load OBJ geometry	// Load OBJ geometry
	vector<float> posBuf;
	vector<float> norBuf;
...
static void printMatrix(const float *A, const char *name = 0)
{
...
// Sets M to be the zero matrix
static void createZeroMatrix(float *M)
{
	memset(M, 0, 16*sizeof(float));
}

// Sets M to be the identity matrix
static void createIdentityMatrix(float *M)
{
	//
	// IMPLEMENT ME
	//
...
// Sets M to be a translation matrix
static void createTranslationMatrix(float *M, float x, float y, float z)
{
	//
	// IMPLEMENT ME
	//
...
// Sets M to be a scale matrix
static void createScaleMatrix(float *M, float x, float y, float z)
{
	//
	// IMPLEMENT ME
	//
...
// Sets M to be an X rotation matrix
static void createRotationXMatrix(float *M, float angle)
{
	//
	// IMPLEMENT ME
	//
...
// Sets M to be a Y rotation matrix
static void createRotationYMatrix(float *M, float angle)
{
	//
	// IMPLEMENT ME
	//
...
// Sets M to be a Z rotation matrix
static void createRotationZMatrix(float *M, float angle)
{
	//
	// IMPLEMENT ME
	//
...
// Sets C = A*B
static void multMatrix(float *C, const float *A, const float *B)
{
	createZeroMatrix(C);
	for(int k = 0; k < 4; ++k) {
...
static void render()
{
	...
	mat4 P;
	P = perspective((float)(45.0*M_PI/180.0), aspect, 0.01f, 100.0f);
	...
	glUniformMatrix4fv(unifIDs["P"], 1, GL_FALSE, &P[0][0]);
	...
	// Send the modelview matrix and draw
    
	createZeroMatrix(A);
    createIdentityMatrix(A);
    createTranslationMatrix(A, 0, 0, -2);
	glUniformMatrix4fv(unifIDs["MV"], 1, GL_FALSE, A);
